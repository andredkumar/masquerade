...
22. ~~**Image-batch output mislabel**~~ — **DONE (2026-09-04).** *This entry's original description was wrong and is rewritten here rather than struck through; the old `videoProcessor.ts:1801` reference was stale (the encoder was at `:2059`).* `processImages` did **not** always encode PNG — it has two branches, and the mislabel ran in **both directions**. The main volumetric path (`:848 → processFrameBatch → :1786`) already honored `outputSettings.format` after the 2B addendum, so a default-settings job wrote **JPEG bytes into `image_001_photo.png`**; only the `catch` fallback (`:923 → processFrame → :2059`) was unconditional PNG, writing **PNG bytes into `image_001_photo.jpg`**. Single root cause either way: `saveProcessedImage` derived the extension from the *upload* (`templateMaskFolderManager.ts:84`), never from the encoder. The lie propagated into the export ZIP, whose entry names come from the on-disk extension (`routes.ts:675`/`:805`), so the default path shipped `images/frame_000000.png` full of JPEG bytes inside the training-data artifact. **Fix:** the fallback encoder now honors `outputSettings.format` with the batch encoder's constants (JPEG q90 default, PNG `compressionLevel: 3` opt-in), and `saveProcessedImage` takes a required `outputFormat` arg and names from it, keeping the upload's basename — mirroring the video path's `ext` at `videoProcessor.ts:516`. Readers needed no change (`listFrameFiles`/`countFrames`/`getProcessedImages` already accept `png|jpg|jpeg`; `mimeForFrameFile` derives the type). Regression test: `server/services/__tests__/saveProcessedImage.test.ts` (8 upload-ext × output-format cases, `npx tsx`, no DB). Pre-fix files on disk are **not** migrated — `mimeForFrameFile`'s PNG fallback keeps them viewable for their retention window. Recon + decision: `docs/refactor/ITEM22_RECON.md`.
